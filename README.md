...
MVC模式中，控制器就是中介者
## 17_chain of responsibility 责任链模式
是一种行为设计模式，允许你将请求沿着处理者链进行发送，收到请求后，每个处理者都可对请求进行处理，或者将其传递给链上的下一个处理者。
和`if else`不同的是，责任链可以在运行时自定义处理者的顺序，而`if else`只能硬编码。
## 18_iterator 迭代器模式
是一种行为设计模式，让你能在不暴露复杂数据结构内部细节的前提下遍历其中所有元素。因为C++STL的存在，迭代器模式在C++已经很少使用。
## 19_memento 备忘录模式
是一种行为设计模式，允许在不暴露对象实现细节的情况下保存和恢复对象之前的状态。当直接访问对象的成员变量、 获取器或设置器将导致封装被突破时， 可以使用该模式。
## 20_filter 过滤器模式
是一种结构型设计模式，它允许通过一系列条件来过滤复杂对象，并只返回满足条件的对象子集。这种模式主要用于根据特定标准从一组对象中过滤出所需的对象。
## 21_visitor 访问者模式
是一种行为设计模式，允许你在不修改已有代码的情况下，向已有类层次结构中添加新的行为。对复杂对象结构（对象树、场景树）的所有元素执行操作时，可以使用访问者模式。
## 22_prototype 原型模式
是一种创建型设计模式，是你能够复制对象，甚至是复杂对象，而又无需使代码依赖它们所属的类。所有的原型类都必须有一个通用的接口，使得即使在对象所属的具体类未知的情况下也能复制对象。原型对象可以生成自身的完整副本，因为相同类的对象可以相互访问对方的私有成员变量。
# 二、软件架构
## 01_ecs
